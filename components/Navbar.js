    ...
    <nav className='bg-gray-900 text-white p-4 px-8 flex justify-between gap-10 items-center'>
      <Link href="/">
        <div className="logo flex justify-center items-center gap-5">
          <Image className='size-12' width={48} height={48} src="/chai.gif" alt="error" />
      ...
      <ul className='flex justify-between items-center gap-10'>
        ...
        <li className='flex gap-2 relative'>
          ...
          {session && <Link href={session.user.name}><Image src={session.user.image} alt="error" className="w-10 rounded-full" /></Link>}
        </li>
      </ul>
    </nav>
  )
